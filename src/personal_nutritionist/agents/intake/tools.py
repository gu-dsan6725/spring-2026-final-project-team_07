...
def update_goal(user_id: str, goal: str) -> dict:
    ...
    result = replace_memory_field(user_id, "goal", f"User's nutrition goal is {goal}.", goal)
...
def update_body_stats(
    ...
    if weight_lbs is not None:
        replace_memory_field(user_id, "weight_lbs", f"User weighs {weight_lbs} lbs.", weight_lbs)
        stored.append(f"weight_lbs={weight_lbs}")
    if height_in is not None:
        replace_memory_field(user_id, "height_in", f"User's height is {height_in} inches.", height_in)
        stored.append(f"height_in={height_in}")
    if age is not None:
        replace_memory_field(user_id, "age", f"User is {age} years old.", age)
        stored.append(f"age={age}")
    if sex is not None:
        if sex not in {"male", "female"}:
            return {"status": "error", "message": "sex must be 'male' or 'female'"}
        replace_memory_field(user_id, "sex", f"User's sex is {sex}.", sex)
...
def update_activity_level(user_id: str, activity_level: str) -> dict:
    ...
    replace_memory_field(user_id, "activity_level", f"User's activity level is {activity_level}.", activity_level)
...
def update_dietary_preferences(
    ...
    if preferred_categories is not None:
        replace_memory_field(user_id, "preferred_categories",
            f"User's preferred meal categories are: {', '.join(preferred_categories)}." if preferred_categories else "User has no preferred categories.",
            preferred_categories)
        stored.append("preferred_categories")
    if disliked_ingredients is not None:
        replace_memory_field(user_id, "disliked_ingredients",
            f"User dislikes these ingredients: {', '.join(disliked_ingredients)}." if disliked_ingredients else "User has no disliked ingredients.",
            disliked_ingredients)
        stored.append("disliked_ingredients")
    if allergies is not None:
        replace_memory_field(user_id, "allergies",
            f"User is allergic to: {', '.join(allergies)}." if allergies else "User has no allergies.",
            allergies)
...
def update_meal_constraints(
    ...
    if max_cost_per_serving is not None:
        replace_memory_field(user_id, "max_cost_per_serving", f"User's maximum cost per serving is ${max_cost_per_serving:.2f}.", max_cost_per_serving)
        stored.append(f"max_cost_per_serving={max_cost_per_serving}")
    if max_total_time is not None:
        replace_memory_field(user_id, "max_total_time", f"User wants meals that take no more than {max_total_time} minutes.", max_total_time)
        stored.append(f"max_total_time={max_total_time}")
    if max_ingredient_count is not None:
        replace_memory_field(user_id, "max_ingredient_count", f"User prefers recipes with at most {max_ingredient_count} ingredients.", max_ingredient_count)
        stored.append(f"max_ingredient_count={max_ingredient_count}")
    if meals_per_day is not None:
        replace_memory_field(user_id, "meals_per_day", f"User eats {meals_per_day} meals per day.", meals_per_day)
